...
const mongoAtlas = "mongodb+srv://palrajan196_db_user:e45d1vl2k4Ai1MMh@expense-cluster.hx0vuld.mongodb.net/";
  const d = new Date();
 const month = d.toLocaleString("default", { month: "long" });
  ...
  const addExpense = async () => {
    await fetch(`${Backend_URL }/expense`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  return (
    <div>
      ...
      >
        ...
        <button type='button' id='editButton' onClick={() => editExpense(editId)} disabled={addEditButton}>
        Edit
      </button>
      </form>
      ...
      <table className="styled-table">
        <thead>
          <tr>
            <th>Date</th>
            <th>Title</th><th>Amount</th><th>Paid By</th><th>Action</th>
          </tr>
        </thead>
        <tbody>
          {expenses.map((e) => (
            <tr key={e._id}>
              <td>{formatDate(e.date)}</td><td>{e.title}</td>
              <td>₹{e.amount}</td><td>{e.paidBy}</td>
              <td>
                <button id='editButton' onClick={() => edit(e)}>Edit</button>
              </td>
            </tr>
          ))}
        </tbody>
      ...
      {balance.result &&
        Object.keys(balance.result).map((user) => (
          <p key={user}>
            <span id='userName'>{user}</span>: {balance.result[user] > 0 ? "+" : ""}
            {balance.result[user] > 0 ? <span id='userPositive'>{balance.result[user].toFixed(2)}</span> : <span id='userNegative'>{balance.result[user].toFixed(2)}</span>}
